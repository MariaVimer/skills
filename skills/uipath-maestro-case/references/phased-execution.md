...
4. Placeholder tasks + unresolved resources — list every placeholder (TaskId, type, display-name, stage) + external resource user must register (task-type-id / connection-id) + wiring-notes from `tasks.md`. Also list **resources built inline** (agents / RPA processes / API workflows — built as in-solution siblings, already bound) and any **built but unreferenced** (reject case) separately. Agent / API-workflow / **functional** RPA siblings need no user action; a **built-unverified** RPA sibling (Windows-target or side-effecting) is bound but **flag that its runtime is unverified** (verify on a Windows host or via full deploy). A **blocked** resource is a placeholder in the list above, not here. See [placeholder-tasks.md § Completion-Report Shape](placeholder-tasks.md#completion-report-shape).
...
> **Resources built inline are not placeholders.** When the user picks **Create** at the Rule 17 gate, Phase 1 builds the resource (a side effect — spawns a sub-agent invoking `uipath-agents` / `uipath-rpa` / `uipath-api-workflow`, registers the sibling, binds it) so it enters Phase 2 as a fully resolved task. Phase 3 never upgrades it (nothing to upgrade). Only resources the user declined/skipped or whose build was **blocked** (needing a live application, credential, or a business rule the SDD never states) become placeholders. (A **built-unverified** RPA sibling — e.g. a Windows-target one, or a side-effecting automation — is still a resolved, bound task, just flagged for runtime verification; not a placeholder.) See [registry-discovery.md § Create-on-Missing](registry-discovery.md#create-on-missing-build-and-rediscovery).
