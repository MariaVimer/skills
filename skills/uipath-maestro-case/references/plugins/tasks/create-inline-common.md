...
**Plugging in a new creatable type** (agentic process, …; the `rpa` row below is the worked example): add a "Creating a `<type>` inline" section to the type's `planning.md` that points to this file for Steps 1/1b/3/Failure and supplies only the deltas: a Step 2 brief, a `resourceSubType` row in the [§ Step 3 table](#step-3--binding-invariants), debug-provisioning behavior, and § 3b adopt tokens. The § Step 3 table row is the only edit needed in this file.
...
| agent | `Agent` |
| api-workflow | `Api` |
| rpa | *(none — omit the `resourceSubType` key entirely; see [rpa/planning.md § Step 3](rpa/planning.md#step-3--binding-drops-resourcesubtype))* |
| *(future: agentic process — added with its leg)* | |
...
## Build result — functional / built-unverified / blocked

The type skill authors the resource's **real internal design** from the Purpose (agent: prompt/tools; api-workflow: activities/connectors; rpa: workflow activities). A sibling is bound as a **resolved** task only when it actually implements its stated behavior — never a stub with default-only outputs. Classify the build result:

| Status | Meaning | Binding |
|--------|---------|---------|
| **functional** | Meets the type skill's completion bar: real logic implementing the Purpose, pinned I/O exact, the type's own validation clean (agent/api-workflow: `validate`; rpa: per-file `validate` **+ project `build`**), and — when the automation is side-effect-free and runnable in the build sub-agent — a passing run. (Agent/api-workflow have no build-time run; their validation bar IS their functional bar.) | Resolved task. |
| **built-unverified** | Real logic + validation/build clean, but runtime could not be safely or possibly verified here — e.g. an rpa **Windows**-target sibling on a non-Windows/Robot-less host, or a side-effecting automation not safe to run in a sub-agent. | Resolved task — **flag "not runtime-verified" in the completion report.** |
| **blocked** | The sub-agent could not author the real logic because something needed is missing: a live application + selectors, a credential/connection, or a business rule the SDD never states. It reports the blocker and does NOT ship a stub that pretends to work. | **NOT resolved → placeholder** (per § Failure); the report names the blocker so the user can supply it and finish the resource. Bind a stub only on **explicit user acceptance**, never silently. |

**Backward compatibility with the `{built}` return.** A leg still returning the older `{built: bool}` maps cleanly: `built:true` → **functional** (its completion bar is the type's validation — api-workflow `validate` Valid, agent config complete), `built:false` (or a dead sub-agent) → **blocked**. So the binding rule generalizes the existing one — **functional / built-unverified → bind as resolved** (built-unverified flagged); **blocked → placeholder** — and only a leg that distinguishes the runtime-verified case (rpa) needs to emit the richer `status`. Agent and api-workflow are unchanged by this section.

## Failure — surface and re-prompt, never stall

Mirrors [connector-integration.md § Creating a Connection](../../connector-integration.md#creating-a-connection) step 4. If a build sub-agent returns **`blocked`** (equivalently `built:false`, or dies), show its `error`/blockers verbatim, then AskUserQuestion: `Retry create` / `Skip (defer)`. On `Skip` or repeated failure, fall to the type's Unresolved Fallback (placeholder + completion-report note) and finish planning — never halt. A verify-time I/O mismatch is a **warning**, not a failure: rewire matched fields, report missing/extra, continue.
