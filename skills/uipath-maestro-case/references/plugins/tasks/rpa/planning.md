...
**The skill does not run `uip rpa init` itself.** It spawns a sub-agent that invokes the `uipath-rpa` skill — RPA-build knowledge lives there. Cross-skill invocation is allowed for this path (overrides the `SKILL.md` "never auto-invoke other skills" anti-pattern). The sub-agent authors a **functional** workflow — real activities implementing the Purpose, following the full `uipath-rpa` authoring workflow — exactly as the agent and api-workflow legs author real behavior. It is **not** a logic-less scaffold. For the **prompt-definable class** (text transforms, JSON parsing, calculations with given formulas, HTTP/API calls to defined endpoints, file ops with explicit formats) it builds, compiles (`uip rpa build`), and — when side-effect-free — runs a working workflow → `functional`. Where the Purpose needs something unavailable to a headless sub-agent — a live desktop/web application + selectors, a credential/connection, or a business rule the SDD never states — it **reports the blocker (`blocked`) instead of shipping a stub**, and the task degrades to a placeholder (never a silently-bound fake). The status model + binding rule are shared — [create-inline-common.md § Build result](../create-inline-common.md#build-result--functional--built-unverified--blocked). **Only processes the user selected at the gate are built — never from SDD content alone.**
...
> **Headless-build reality (drives the result status).** The sub-agent has no Robot but does have .NET (`DOTNET_ROOT`). A **Portable** sibling for prompt-definable work is authored, compiled (`uip rpa build`), and — when side-effect-free — run here → `functional`. A **Windows**-target sibling can be authored with real activities but **cannot compile or run on this non-Windows, Robot-less host** → its result is `built-unverified` (real logic; build/run deferred to a Windows host — flagged in the completion report), NOT a failure and NOT a stub. A task whose Purpose needs a live application + selectors, a credential/connection, or an unstated business rule → `blocked` (report it; the task placeholders). See [create-inline-common.md § Build result](../create-inline-common.md#build-result--functional--built-unverified--blocked).
...
do not publish/upload/deploy.
  ...
  Then VERIFY, escalating (this determines the result status):
    ...
    - Project `uip rpa build "<DIR>" --output json` until clean — compile catches the
      member/enum/XAML errors `validate` misses (now that REAL activities are authored,
      this is REQUIRED for a Portable target). A Windows target cannot compile on this
      non-Windows/Robot-less host → skip build and return status:"built-unverified".
    - When the automation is side-effect-free (pure text/JSON/calc, or an idempotent read),
      ...
      produces sane outputs → status:"functional". Do NOT run a side-effecting automation
      (writes/sends/external mutations) → status:"built-unverified".
  ...
  invent it or emit a stub. Return status:"blocked" with the specific blocker(s) named.
  ...
  uipath-rpa skill, do NOT improvise — return status:"blocked" with the reason.
Return JSON: { status: "functional"|"built-unverified"|"blocked", path,
  finalInputs:[{name,type}], finalOutputs:[{name,type}], validationPassed: bool,
  buildPassed: bool, runtimeVerified: bool, blockers: [<string>], error? }
```

The brief is self-contained — it carries the Step-1b Purpose and the pinned I/O, and no other case context (do not dump `caseplan.json` or sibling tasks). Building runs in a sub-agent; orchestration/parallelism per [registry-discovery.md § Create-on-Missing](../../../registry-discovery.md#create-on-missing-build-and-rediscovery). The **caller registers** the built sibling (`uip solution project add`, then `resources refresh`) before rediscovery. `--target-framework` carries the § Target-framework choice — do **not** force Portable; because the flag is immutable after `init`, a mismatch costs the user a full recreate, not a retarget. Result status → [create-inline-common.md § Build result](../create-inline-common.md#build-result--functional--built-unverified--blocked): **functional** (Portable compute — built + run here) and **built-unverified** (Windows target, or a side-effecting automation — real logic, runtime deferred) both bind as resolved (unverified flagged in the completion report); **blocked** (missing app/selectors/credential/rule) placeholders. The optional full-deploy verification (Portable, serverless) is owned by [phased-execution.md § Debug notes](../../../phased-execution.md#debug-notes); a Windows sibling is deployed and run by the user on a Windows runtime.
...
The orchestration owns rediscover → verify → bind ([registry-discovery.md § Create-on-Missing § 4](../../../registry-discovery.md#create-on-missing-build-and-rediscovery) — incl. exact-name matching, warn-don't-block, and `EntityKey` audit-only). RPA deltas only: the rediscovery token is **`--type process`** (§ sibling check above), and RPA siblings have **no `entry-points.json`** — read the case-preserving argument names + .NET types from the on-disk **`project.json` `entryPoints[].input/output`** (never from the PascalCased `--local` `Resource.{Inputs,Outputs}`), reconciling .NET→case via the Step-1 map. **Known rename trap:** a builder that honors the RPA analyzer's `^in_`/`^out_` argument-prefix naming warning prefixes arguments `in_`/`out_`, turning every pinned name into missing+extra — the § 4 exact-name rule applies (report the diff; never "convention-match"); the Step-2 brief forbids that rename at the source. Bind by status per [create-inline-common.md § Build result](../create-inline-common.md#build-result--functional--built-unverified--blocked): **functional**/**built-unverified** → resolved (unverified flagged); **blocked** → placeholder.

### Failure — surface and re-prompt, never stall

Shared contract — [create-inline-common.md § Failure](../create-inline-common.md#failure--surface-and-re-prompt-never-stall): `blocked` / `built:false` → show blockers verbatim → AskUserQuestion `Retry create` / `Skip (defer)` → on Skip/repeat, Unresolved Fallback above. **If a blocker is a missing business rule or an available connection the user can supply, they may provide it and Retry** (the re-brief carries it). A `functional` / `built-unverified` result binds as a resolved task (unverified flagged) — never placeholdered. Verify-time I/O mismatch = warning, never a failure.
