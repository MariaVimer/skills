...
First ask a **yes/no** question — **Open the flow in editor?** On **Yes**, run `code -r "<ProjectName>.flow"` (`.flow` opens in the visual Flow editor by default — no viewType/flags; if `code` isn't on `PATH`, try `code-insiders` then `cursor`; if none resolve, report the file path; skip in headless/CI). On **No**, continue. Independent of the next action — opening the flow does not replace publish/debug/deploy.
