...
**Controlling package sources.** In offline or air-gapped environments, or when packages must come from specific feeds, pass `--nuget-sources-config-path <path>` pointing at a local `NuGet.config`. Both `pack` and `restore` accept it: the sources declared in that file then control where each project's packages are resolved from, so feed selection stays fully in your hands.
