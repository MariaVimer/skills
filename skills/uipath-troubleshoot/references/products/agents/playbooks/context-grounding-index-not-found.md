...
**If the index was deleted — re-create it:**
  ...
  Wait for `last_ingestion_status` to become `Successful`. If the solution package contains generated index resources, refresh and upload from the solution root so the current index and bucket keys are embedded:
