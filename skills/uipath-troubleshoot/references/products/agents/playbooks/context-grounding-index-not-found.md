...
**If the index was deleted — re-create it:**
  ...
  Wait for `last_ingestion_status` to become `Successful`. No agent project change or publication is needed when the existing resource reference already names this index and folder; the runtime resolves it directly.
